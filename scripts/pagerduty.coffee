...
#   hubot pagerduty trigger <msg> - create a new incident with <msg>
#   hubot pagerduty 60 - take the pager for 60 minutes
#   hubot pagerduty as <email> - remember your pager email is <email>
#   hubot pagerduty incidents - return the current incidents
#   hubot pagerduty incident NNN - return the incident NNN
#   hubot pagerduty note <incident> <content> - add note to incident #<incident> with <content>
#   hubot pagerduty notes <incident> - show notes for incident #<incident>
#   hubot pagerduty problems - return all open incidents
#   hubot pagerduty ack <incident> - ack incident #<incident>
#   hubot pagerduty resolve <incident1> <incident2> ... <incidentN> - ack all specified incidents
#   hubot pagerduty ack - ack all triggered incidents
#   hubot pagerduty resolve <incident> - resolve incident #<incident>
#   hubot pagerduty resolve <incident1> <incident2> ... <incidentN>- resolve all specified incidents
#   hubot pagerduty resolve - resolve all acknowledged incidents
#
# Dependencies:
#  "moment": "1.6.2"
#
...
module.exports = (robot) ->
  robot.respond /pager(duty)?$/i, (msg) ->
    ...
    withPagerDutyUser msg, (user) ->
      emailNote = if msg.message.user.pagerdutyEmail
                    ...
                    "I'm assuming your PagerDuty email is #{msg.message.user.email_address}. Change it with `#{robot.name} pagerduty as you@yourdomain.com`"
      if user
    ...
    cmds = (cmd for cmd in cmds when cmd.match(/(pagerduty |who's on call)/))
    msg.send cmds.join("\n")

  robot.respond /pager(?:duty)? as (.*)$/i, (msg) ->
    email = msg.match[1]
  ...
  robot.respond /pager(duty)? (\d+)/i, (msg) ->
    withPagerDutyUser msg, (user) ->
  ...
  robot.respond /(pager)(duty)? incident (.*)$/, (msg) ->
    pagerDutyIncident msg, msg.match[3], (incident) ->
      msg.send formatIncident(incident)

  robot.respond /(pager)(duty)? (inc|incidents|sup|problems)$/i, (msg) ->
  ...
  robot.respond /(pager)(duty)? (?:trigger|page) (.+)$/i, (msg) ->
  ...
  robot.respond /(?:pager)(?:duty)? ack(?:nowledge)? (.+)$/i, (msg) ->
  ...
  robot.respond /(?:pager)(?:duty)? res(?:olve)?(?:d)? (.+)$/i, (msg) ->
    incidentNumbers = parseIncidentNumbers(msg.match[1])
  ...
  robot.respond /(pager)(duty)? notes (.+)$/i, (msg) ->
  ...
  robot.respond /(pager)(duty)? note ([\d\w]+) (.+)$/i, (msg) ->
    incidentId = msg.match[3]
    content = msg.match[4]
  ...
  withPagerDutyUser = (msg, cb) ->
    ...
    unless email
      msg.send "Sorry, I can't figure out your email address :( Can you tell me with `#{robot.name} pagerduty as you@yourdomain.com`?"
      return
